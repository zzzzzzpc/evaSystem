...
<script>
	...
	import Vue from 'vue';
	import asynchronous from '../js/asynchronous.js';
	Vue.prototype.$asynhttp = asynchronous;
	export default {
		name:"login_vue",
		data(){
			return {
		...
		methods:{
			loginValidate(flag){
				...
				this.$asynhttp.postData('/login/',param)
				.then(response => {
					// console.log("2323"+response);
					this.booldata = response;
					if(response=="True"){
						// this.$router.push({ name: '/stuinterface' ,params:this.sno})
				...
				// 	booldata = response;
				// 	console.log("000"+booldata);
				// 	console.log("2323"+response);
				// });
				// console.log("111"+this.booldata);
				// if(this.booldata=="False"){	//用户名或密码错误
...
<style>
	.login_body {
		...
		/* filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src='../img/mainBg.jpg', sizingMethod='scale'); */
